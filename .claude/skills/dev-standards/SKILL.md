...
2. **AGENTS.md**：在專案根目錄建立，作為所有 AI 工具遵循的開發與協作規範。內容應涵蓋：專案技術棧、目錄結構慣例、程式風格、commit 規範、禁止事項（如機密資訊）、如何執行測試/建置。若專案需要相容 Claude Code，額外建立 `CLAUDE.md`，內容應指向或同步 `AGENTS.md` 的規範，不要重複維護兩份不一致的內容。
